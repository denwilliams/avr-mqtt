...
exports.create = (avrClient, state) => {
  ...
  return {
    setPower(on) {
      ...
      .then(() => {
        if (on) return avrClient.powerOff(1);
        else return avrClient.powerOn(1);
      })
      .then(() => {
    ...
    mute() {
      return avrClient.muteOn(1)
      .then(() => {
        state.set(KEYS.MUTE, true);
      });
    },
    unmute() {
      return avrClient.muteOff(1)
      .then(() => {
    ...
    setInput(input) {
      return avrClient.setInputTo(input, 1)
      .then(() => {
