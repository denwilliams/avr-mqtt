...
exports.start = () => {
  ...
  mqttClient.on('message', (topic, message) => {
    ...
    const [deviceId, key, action] = deviceTopic.split('/');

    const strMsg = message.toString();
    const value = strMsg ? JSON.parse(strMsg) : undefined;

    emitter.emit('message', { deviceId, key, action, value });
  });
...
exports.emit = (device, event, data) => {
  if (!mqttConnected) return;

  const topic = `${topicPrefix}/${device.id}/${event}`;
